...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            <h2 class="text-3xl font-black text-yellow-500 uppercase tracking-widest mb-1">Edit Pelanggan</h2>
            <p class="text-zinc-500 tracking-widest text-[10px] font-bold uppercase">Memperbarui informasi: {{ $customer->name }}</p>
        </div>
            ...
            <form action="{{ route('customers.update', $customer) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="address" rows="3"
                          class="w-full bg-zinc-950 border border-zinc-800 rounded-none px-4 py-3 text-zinc-200 focus:outline-none focus:border-zinc-500 transition-all placeholder-zinc-700 text-sm">{{ old('address', $customer->address) }}</textarea>
                    @error('address')
                        <p class="text-red-500 text-[10px] font-bold mt-1 uppercase italic tracking-widest">{{ $message }}</p>
                    @enderror
                </div>
